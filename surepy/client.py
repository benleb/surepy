...
class SureAPIClient:
    ...
    #         await self.call(method="GET", resource=MESTART_RESOURCE)
    ...
    #             # print(f"{p.name = }")
    #             # print(f"{p.pet_id = }")
    #             # print(f"{p.household_id = }")
    #             # print(f"{p.tag_id = }")
    #             # print(f"{p.updated_at = }")
    #             # print(f"{p.photo_url = }")
    #             # print(f"{p.food_type = }")
    #             # print(f"{str(p.location) = }")
    #             # print(f"{str(p.activity) = }")

    #             entities[entity["id"]] = entity

    #     return entities

    # def data_reader(self, type: str, data: Dict[str, Any]) -> Dict[str, Any]:

    #     if type == "pets":

    #         position = data.get("position", {})
    #         pet_location: PetLocation = PetLocation(
    #             where=Location(position.get("where", Location.UNKNOWN.value)),
    #             since=position.get("since", None),
    #         )

    #         activity = data.get("activity", {})
    #         pet_activity: PetActivity = PetActivity(
    #             where=Location(activity.get("where", Location.UNKNOWN.value)),
    #             since=activity.get("since", None),
    #         )

    #         food_type: Optional[FoodType] = None
    #         if food_type_id := data.get("food_type_id"):
    #             food_type = FoodType(food_type_id)

    #         return {
    #             "pet_id": int(data.get("id", None)),
    #             "household_id": int(data.get("household_id", None)),
    #             "name": str(data.get("name", None)),
    #             "photo_url": urlparse(data.get("photo", {}).get("location", None)).geturl(),
